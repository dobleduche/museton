...
export const SettingsModal: React.FC<Props> = ({ prefs, onUpdate, onClose }) => {
  ...
  const handleClearData = () => {
    if (
      window.confirm(
        'DANGER: This will wipe all local projects and presets. Continue?'
      )
    ) {
      localStorage.clear()
          ...
          <div className="w-48 bg-black/20 border-r border-white/5 p-4 space-y-1">
            {[
              ...
              <button
                key={tab.id}
                onClick={() =>
                  setActiveTab(tab.id as 'TOOLS' | 'AUDIO' | 'DATA')
                }
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4 bg-slate-800 p-3 rounded-lg">
                    <input
                      ...
                      onChange={(e) =>
                        onUpdate({
                          masterTuning: Number(e.target.value),
                        })
                      }
                ...
                <div className="flex items-center justify-between p-3 bg-slate-800 rounded-lg">
                  <div>
                    ...
                    <div className="text-xs text-slate-500">
                      Reduce visual effects for lower latency
                    </div>
                  </div>
                  <button
                    onClick={() =>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-600 mt-2 text-center">
                    Irreversible. Clears all IndexedDB and LocalStorage
                    data.
                  </p>
                </div>
              </div>
